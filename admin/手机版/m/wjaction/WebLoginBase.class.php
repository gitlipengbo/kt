...
class WebLoginBase extends WebBase{
	...
	function __construct($dsn, $user='', $password=''){
		...
		try{
			...
			// 限制同一个用户只能在一个地方登录
			//var_dump("select isOnLine from wj5ssc_member_session where id={$this->user['sessionId']}");exit;
			if(!$this->getValue("select isOnLine from wj5ssc_member_session where uid={$this->user['uid']} and session_key=? order by id desc limit 1", session_id())){
				header('location: /index.php/user/login');
				exit('您已经退出登录，请重新登录');
			}
		}catch(Exception $e){
	...
	public function addCoin($log){
		...
		$sql="call setCoin({$log['coin']}, {$log['fcoin']}, {$log['uid']}, {$log['typea']}, {$log['liqType']}, {$log['type']}, '{$log['info']}', {$log['extfield0']}, '{$log['extfield1']}', '{$log['extfield2']}')";
