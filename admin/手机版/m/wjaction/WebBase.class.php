...
class WebBase extends Object{
	...
	public function getPlayeds(){
		...
		$sql="select * from {$this->prename}played where android=1";
	...
	public function getPl($type=null, $played=null){
		...
		$sql="select bonusProp, bonusPropBase from {$this->prename}played where android=1 and id=?";
		//echo $sql;
	...
	public function no00Hd(&$actionNo, &$actionTime, $time=null){
		$this->setTimeNo($actionTime, $time);
	...
	public function updateSessionTime(){
		$sql="update wj5ssc_member_session set accessTime={$this->time} where id=?";
