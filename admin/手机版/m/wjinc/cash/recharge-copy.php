...
$mBankId=intval($args[0]['mBankId']);
$sql="select mb.username, mb.account, mb.rechargeDemo, b.name bankName, b.logo bankLogo, b.home bankHome from {$this->prename}admin_bank mb  left join {$this->prename}bank_list b on b.id=mb.bankId where mb.enable=1 and mb.bankId=$mBankId";
...
<dl class="wjform" id="display-dom">
    <dt>银行类型：</dt>
    <dd><img id="bank-type-icon" class="bankimg" src="<?=$GLOBALS['siteUrl']?>/<?=$memberBank['bankLogo']?>" title="<?=$memberBank['bankName']?>" />
    <a id="bank-link" target="_blank" href="<?=$memberBank['bankHome']?>" class="ml10">进入银行网站>></a></dd>
</dl>
<dl class="wjform" id="display-dom">
    <dt>银行账号：</dt>
    <dd><input id="bank-account" readonly value="<?=$memberBank["account"]?>" style="width:220px;" /><div class="btn-a copy" for="bank-account">
...
<dl class="wjform" id="display-dom">
    <dt>账户名：</dt>
    <dd><input id="bank-username" readonly value="<?=$memberBank["username"]?>" />
...
<dl class="wjform" id="display-dom">
    <dt>充值金额：</dt>
    <dd><input id="recharg-amount" readonly value="<?=$args[0]['amount']?>" />
...
<dl class="wjform" id="display-dom">
    <dt>&nbsp;</dt>
    <dd class="red">[*网银充值金额必须与网站填写金额一致方能到账！]</dd>
</dl>
<dl class="wjform" id="display-dom">
    <dt>充值编号：</dt>
    <dd><input id="username" readonly value="<?=$args[0]['rechargeId']?>" />
...
<dl class="wjform" id="display-dom">
    <dt>&nbsp;</dt>
    <dd class="red">[*每个充值编号仅用于一笔充值，重复使用将不能到账！]</dd>
</dl>
