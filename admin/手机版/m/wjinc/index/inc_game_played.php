<?php
	$sql="select groupName from {$this->prename}played_group where android=1 and id=?";
	$groupName=$this->getValue($sql, $this->groupId);

	$sql="select id, name, playedTpl, enable  from {$this->prename}played where enable=1 and android=1 and groupId=? order by sort";
