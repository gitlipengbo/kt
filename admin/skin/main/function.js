...
function gameKanJiangDataC(diffTime, actionNo){
	...
	var thisNo=$('.b-top-tips-cur span').html();
	var tips='本期['+thisNo+']已截至投注';
	if(diffTime<=0){
		
		if($('#kaijiang .feipan')) $('#kaijiang .feipan em').html("X");
		$dom.text('<span>00</span><em>:</em><span>00</span><em>:</em><span>00</span>');
		$('#kjinfo').html('正在封单中');
		...
		$('.b-top-info span.red').text($('.b-top-tips-cur span').text());
		setKJWaiting(kjTime);
	}else{
	...
	if(s>60){
		h=Math.floor(s/60);
		s=s-h*60;
		$dom.html("<span>"+(h<10?"0"+h:h)+"</span><em>:</em><span>"+(s<10?"0"+s:s)+"</span><em>:</em><span>"+m+"</span>");

	}else{
		h=0;
		$dom.html("<span>00</span><em>:</em><span>"+s+"</span><em>:</em><span>"+m+"</span>");
	}
	if(S && h==0 && m==6 && s==0){
...
function getQiHao(){
	$.getJSON('/index.php/index/getQiHao/'+game.type, function(data){
		if(data && data.lastNo && data.thisNo){
			...
			$('#curExpectSpan').html(data.thisNo.actionNo);
				...
				try{
					...
					var times=3000;
						if(ctype=='g1'){
							...
							$('.kj-hao li').each(function(i){
								times-=500;
								setTimeout($(this).html(hao[i]),times);
							});
						}else{
							$('.kj-hao li').each(function(i){
								times-=500;
...
function reloadMemberInfo(){
	$('.toolbar').load('/index.php/index/userInfo');
}
