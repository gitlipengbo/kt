	...
	var pros = {
		...
		getMethodByName: function(serviceData){
			...
			for (; i < serviceData.length; i++) {
				...
				}else if(data.length>0 && nameArr[0] == 'game' && nameArr[1] == 'middle'){
					//游戏中部公告
		 ...
		 if(!$.cookie(uname+'_Notice_Close'))
		 {
           if($('body').find('.messageNotice').length == 0)
		   {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(!$.cookie(key.join('')))
					{
					html.push('<li><a href="javascript:void(0);" endTime='+data[i]['endTime']+' pid='+data[i]['url']+' url='+me.defConfig.logOut+'/ad/adNoticeQuery?id=' + data[i]['url'] + '>' + data[i]['text'] + '</a></li>');
					}
					key=[];
				};
				html.push('</ul>');
		...
		gameMessageNotice: function(data){
			...
			html.push('<ul>');
			for (var i = 0; i < data.length; i++) {
				html.push('<li><a href="/ad/adNoticeQuery?id=' + data[i]['url'] + '">' + data[i]['text'] + '</a></li>');
			};
			html.push('</ul>');
			...
			//输出通知模版
			$('.common-main').before(me['gameMessageNoticeDom']);
			//显示通知栏
		...
		proxyMessageNotice: function(data){
			var me = this,
			    ...
			    key = [],
				html = [];
		 if(!$.cookie(uname+'_Proxy_Close'))
		 {
		  if($('body').find('.proxyMessageNotice').length == 0)
		  {
				html.push('<ul>');
				for (var i = 0; i < data.length; i++) {
						key.push(uname+"_Proxy_"+data[i]['url']);
						if(!$.cookie(key.join('')))
						{
						html.push('<li><a href="javascript:void(0);" endTime='+data[i]['endTime']+' pid='+data[i]['url']+' url='+me.defConfig.logOut+'/ad/adNoticeQuery?id=' + data[i]['url'] + '>' + data[i]['text'] + '</a></li>');
						}
		   ...
		   }else
		   {
			   for (var i = 0; i < data.length; i++) {
			    ...
			    $('body').find('.proxyMessageNotice').find('ul').prepend(html);
...
$(function(){
	var logouturl=(typeof _logOut != "undefined")?_logOut:'';
	
	//通知&&消息提示组件
	var msgNoticeTips = new phoenix.Notice({
		delayTime: 2,
		serviceUrl: logouturl+'/api/jsonp/getNotices?page=' + global_params_notice+"&r="+Math.random(),
		logOut:logouturl
	});
	...
	$('body').on('click', '.proxyMessageNotice .ico-close', function(){
		...
		var uname=global_userID,key=uname+'_Proxy_Close';
		$.cookie(key,true,{path:'/',domain:window.location.hostname.substring(window.location.hostname.indexOf('.'))});
	});
	
	$('body').on('click','.proxyMessageNotice a',function(){
		var me=$(this),pid=me.attr('pid'),url=me.attr('url'),endTime=me.attr('endTime'),unixTime='0'+endTime.substring(0,endTime.length-3),data=new Date(phoenix.util.UnixToDate('Y/m/d H:i:s',unixTime)),uname=global_userID,key=uname+"_Proxy_"+pid;
		if(!$.cookie(key))
		{
		   $.cookie(key,true,{expires:data,path:'/',domain:window.location.hostname.substring(window.location.hostname.indexOf('.'))});
		}
	...
	/*var msgTips = new phoenix.Notice({
		delayTime: 5,
		serviceUrl: logouturl+'/api/jsonp/getTips?userId='+global_userID+'&r='+Math.random(),
		serviceGetType: 'push',
		msgPushWaitTime: 8,
		logOut:(typeof _logOut != "undefined")?_logOut:'' 
	});*/
