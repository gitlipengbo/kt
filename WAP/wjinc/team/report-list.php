...
<?php
	...
	if($para['parentId']=intval($para['parentId'])){
		...
		$userWhere="and (u.parentId={$uid} or u.uid={$uid}) ";  //concat(',', u.parents, ',') like '%,$uid,%'
	}
	$userWhere3="and (u.parentId={$uid} or u.uid={$uid})";
	//$userWhere3="and concat(',', u.parents, ',') like '%,$uid,%'";
	
	$sql="select u.username, u.coin, u.uid, u.parentId, sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount, (select sum(c.amount) from wj5ssc_member_cash c where c.`uid`=u.`uid` and c.state=0 $cashTimeWhere) cashAmount,(select sum(r.amount) from wj5ssc_member_recharge r where r.`uid`=u.`uid` and r.state in(1,2,9) $rechargeTimeWhere) rechargeAmount, (select sum(l.coin) from wj5ssc_coin_log l where l.`uid`=u.`uid` and l.liqType in(50,51,52,53) $brokerageTimeWhere) brokerageAmount from wj5ssc_members u left join wj5ssc_bets b on u.uid=b.uid and b.isDelete=0 $betTimeWhere where 1 $userWhere";
	//echo $sql;exit;
...
<table width="100%" class='table_b'>
	...
	<?php } ?>
        ...
        <?php if(intval($para['userType'])==1 || (intval($para['userType'])==0 && !$para['parentId']) ||($para['username'] && $para['username']!='用户名')){//------------------------------------------
				$sql2="select sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount from wj5ssc_members u left join wj5ssc_bets b on u.uid=b.uid and b.isDelete=0 $betTimeWhere $userWhere3";
		...
		<tr>
			...
			<td><?=$this->ifs($all['zjAmount']-$all['betAmount']+$all['fanDianAmount'], '--')?></td>
			<td></td>
		</tr>
		<?php }else{//----------------------------------------
				$sql2="select sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount from wj5ssc_members u left join wj5ssc_bets b on u.uid=b.uid and b.isDelete=0 $betTimeWhere $userWhere";
				$all=$this->getRow($sql2);
