...
class Business extends AdminBase{
	...
	public final function rechargeModal(){
		...
		public final function rechargeModalyj(){
		$this->display('business/recharge-modalyj.php');
	}
	/**
	 * 每天首次充值佣金活动
	 */
	...
	public final function rechargeAction($uid, $amount,$uIN){
		...
		/**
			 * 佣金充值操作
	 */
	public final function rechargeActionyj($uid, $amount,$uIN){
		...
		try{
			...
			$data['fcoin']=$user['fcoin'];
			$data['username']=$user['username'];
			$data['info']='管理员"'.$this->user['username'].'"发放佣金';
			$data['state']=9;
	...
	*/
	...
	public final function rechargeHandle(){
		...
		//throw new Exception($para['zsAmount']);
		try{
			$this->beginTransaction();
